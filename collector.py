...
def save_to_db(data):
    try:
        # Edit: Path modified for GitHub Actions compatibility
        db_folder = os.path.join(os.getcwd(), 'data')
        if not os.path.exists(db_folder):
            os.makedirs(db_folder)
        
        db_path = os.path.join(db_folder, 'pollution_system.db')
...
if __name__ == "__main__":
    ...
    # Edit: Removed the while loop. GitHub Actions handles the 15-minute interval.
    combined_data = get_complete_data()
    if combined_data:
        save_to_db(combined_data)
        print(f"Success: Data saved at {combined_data['Timestamp']}")
    else:
